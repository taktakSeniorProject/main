...
db.connect((err) => {
  ...
  const itemofuser=(req,res)=>{
   ...
   const additem=(req,res)=>{
    const {user_id,wishlists_wishlist_id}=req.params
    const {title,description,price,gategorie,img}=req.body
    
    try {
      const query2 = `INSERT INTO items (title,description,price,gategorie,img,user_user_id,wishlists_wishlist_id)
       VALUES ("${title}","${description}","${price}","${gategorie}","${img}","${user_id}","${wishlists_wishlist_id}")`
